...
## 验收标准（可测试）
- [ ] init 后 doctor 7 PASS / 0 WARN（目录布局 / 钩子挂载 / managed 台账 / 无占位符残留 / INDEX 无漂移 / check-loop 干净）
- [ ] 既有 workflow/ 留痕原样保留（M1/M3 四份文档内容不变）
- [ ] AGENTS.md 项目适配区填本仓库真实口径（无构建、纯 JS）；`npm test` 跑全 7 套测试（38 + 105）全绿
- [ ] M4 提交穿过真钩子（pre-commit / commit-msg）成功落库；提交后 doctor / check-loop 复跑干净
- [ ] 双源纪律（改 templates/ → sync）落 AGENTS.md 项目适配区
