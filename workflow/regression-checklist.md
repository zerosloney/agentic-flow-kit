...
| 2026-09-25-doctor-owned-drift | doctor 必须校验 kit.owned 列表盘面 sha vs 台账 sha 不一致（drift + gone 双信号）；checkOwnedDrift 函数必须独立 export 供测试套件直接 import；新增校验必须配套 fixture 测试覆盖（场景含对齐/手改/缺失/无 owned/kit.json 不存在/解析失败/多 owned 部分漂移） | doctor.test.mjs 场景 1-7（npm test 随跑） |
