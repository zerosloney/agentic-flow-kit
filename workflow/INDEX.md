...
| INTENT | L1 | approved | 2026-09-25 | pipeline | 装户面同步一致性检查（source-sync-check.mjs） | ☑0/8 |
| PLAN | L1 | approved | 2026-09-25 | pipeline | 装户面同步一致性检查（source-sync-check.mjs） | — |
...
终态构成：done 41 · closed 6 · fixed 3；查档案用 `node .agents/scripts/kb-search.mjs "<词>" --status all`。
