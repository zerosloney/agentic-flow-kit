...
| PLAN | L1 | approved | 2026-09-24 | pipeline | 看板拉起入口跨平台化（ensure-board.mjs 取代 ps1） | — |
| PLAN | L1 | approved | 2026-09-24 | pipeline | 看板端口自动检测可用端口（多项目并行不冲突） | — |
...
终态构成：done 20 · fixed 3 · closed 2；查档案用 `node .agents/scripts/kb-search.mjs "<词>" --status all`。
