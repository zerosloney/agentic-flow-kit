...
1. **编码权威**:FormulaEngine / CategoryPrefix / 编码模板 / 取号计数器 / 编码去重与命名空间
2. **共享契约**:shared 层基类与 DTO 形状、跨模块公共接口签名
...
> 先例:`specs/2026-09-18-l3-global-owned-code-collision.md`(①)、`specs/2026-09-21-shared-paged-query-contract.md`(②)、`intents/2026-09-19-single-comparison-list.md`(③)、`specs/2026-09-19-modelstate-validation-unify.md`(④)。

> **L1 是默认档**:红线全不勾即 L1,无需正列举——判级只回答「是否命中 L2 四类或 L3 结构面」。
> **L3 的「DI 链」按组合根与运行时管线理解**:DI 注册、中间件/认证管线、EF 诊断侦听、服务构造器依赖变更(先例:`intents/2026-09-16-real-jwt-auth.md` 无 schema 变更仍 L3;`intents/2026-09-18-miniprofiler-perf-profiling.md`)。
