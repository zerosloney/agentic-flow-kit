...
Hard constraint (Trae subagents cannot deny specific commands the way the OpenCode permission block does): never run `git commit`, `git push`, or `git reset --hard`; the role contract forbids any commit, push, or destructive git operation. The main agent's PreToolUse hook (`.trae/hooks/pre-shell-check.js`) enforces this — do not assume it, and do not attempt to reach those commands by another route.
