...
stage: review（横切：合入前使用，不占 6 阶段之一）
triggers:
  - "评审一下"
  - "过 review 标准"
  - "按评审标准过一遍"
delegation:
  - role: independent-reviewer
    ...
    when: 机器门禁完成且复核目标与 diff 基准已固定（L2 / L3 强制独立复核,宿主不支持时按正文「独立性兜底」另开新会话,不得回退主智能体）
    required: false
    fallback: new-session
approval_required: true
next: .agents/commands/build.md（仅 P0 / P1 未清时；清零后经用户确认合入）
---
